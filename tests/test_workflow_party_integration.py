...
async def test_workflow_classifies_parties():
    ...
    claims = [
        {"claim": "Test 1", "who": ["US", "America"], "when": "", "where": "", "confidence": "HIGH"},
        {"claim": "Test 2", "who": ["Iran"], "when": "", "where": "", "confidence": "HIGH"}
    ]
    ...
    with patch("src.ai.workflow.collect_claims", new=AsyncMock(return_value=claims)):
        with patch("src.ai.workflow.classify_parties", new=AsyncMock(return_value=party_data)):
            with patch("src.ai.workflow.cluster_claims", new=AsyncMock(return_value={"clusters": {"0": claims}})):
                with patch("src.ai.workflow.narrate_cluster", new=AsyncMock(return_value={
                    "stance_summary": "Test",
                    "key_themes": [],
